...
int main(){
    ...
    while (1){
        ...
        if (Retning == 1 && Bestilling_opp[Siste_Etasje] == 1){
            Retning = Stoppet_i_etasje();
            
        }

        if (Retning == -1 && Bestilling_ned[Siste_Etasje] == 1){
            Retning = Stoppet_i_etasje();
        }
        Oppdater_bestillinger();
        Stoppknapp();
        ...
        int stopp = elevio_stopButton();
        int avbryt = elevio_callButton(0, 2);
        if (avbryt == 1 && stopp == 1){
            return 0;
        }
